'''
1. if 문
    if문은 조건이 참일 경우에만 해당 블록의 코드 실행
2. if- else 문
    if는 조건이 참일 때 실행 / false 일 때는 else 실행
3. if - elif - else 문
'''
...
그래서 수학적인 연산을 위해서는 형변환 함수인 int() / float()을 사용할 필요가 있다.
'''
# age = int(input('당신의 나이를 입력하세요 >>> '))
# input()의 결과값의 자료형은 str이고 그걸 다시 int로 바꾸는 함수가 적용되어 일종의 chaining 메서드처럼 쓸 수 있다.
...
'''
age = 21
has_ticket = True # boolean 자료형일 때 python만 대문자로 시작한다.
